...
Placing the two files at those default paths is enough -- no environment variables are required at all. Note: "requires no external service" applies at runtime only.

**At build time**, the `ort` crate downloads a prebuilt onnxruntime binary (from cdn.pyke.io). If your build environment is also air-gapped, provide a pre-placed onnxruntime and point the build at it with the `ORT_LIB_LOCATION` environment variable.

## `openai` — OpenAI-compatible API

Uses an `/v1/embeddings`-compatible endpoint such as Ollama, LM Studio, or OpenAI. Set `YSR_EMBEDDING_PROVIDER=openai` explicitly to opt into this instead of the local ONNX default:
